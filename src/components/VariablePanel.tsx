...
export default function VariablePanel({ variables, onAddVariable, onRemoveVariable, onVariableChange }: VariablePanelProps) {
    ...
    return (
        <div className="relative h-full">
            <h2 className="text-center text-xl font-bold mb-4">Story State</h2>
            <div className="flex flex-col gap-2">
                { variables.map((variable, index) => (
                    <div key={index} className="grid grid-cols-12 gap-2 items-center">
                        <button className="cursor-pointer" onClick={() => onRemoveVariable(index)}>
                        ...
                        <span className="col-span-1">=</span>
                        {
                            ...
                            :
                                <input
                                    type={variable.type}
                                    className="col-span-3 input-primary"
                                    value={variable.type === 'number' ? Number(variable.defaultValue) : String(variable.defaultValue)}
                                    onChange={(e) => onVariableChange(index, 'defaultValue', e.currentTarget.value)}
                                />
                        }
                    </div>
